...
export async function setupViteMiddleware(app: Application) {
  const vite = await createViteServer({
    ...viteConfig,
    // ⚠️ vite 即使收到 inline config 也会重新加载 vite.config.ts 并 merge，
    // plugins 数组会被合并成两个 react() 实例 → react-refresh preamble 注入两次
    // → esbuild 报 $RefreshReg$/$RefreshSig$ already declared → 所有 .tsx 转换失败 → 白屏
    configFile: false,
    server: {
      ...viteConfig.server,
      middlewareMode: true,
